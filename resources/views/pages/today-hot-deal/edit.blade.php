...
@section('content')
    <div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6">
        ...
        <form action="{{ route('todayHotDeal.update', $todayHotDeal->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="space-y-6">
                                ...
                                <div class="relative z-20 bg-transparent">
                                    <select
                                        ...
                                        @foreach($products as $product)
                                            <option  {{ old('product_id', $todayHotDeal->product_id) == $product->id ? 'selected' : '' }} value="{{ $product->id }}">{{ $product->name }}</option>
                                        @endforeach
                                    </select>
                                    <span
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
                    <a href="{{ route('todayHotDeal.index') }}"
